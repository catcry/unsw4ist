# -*- coding: utf-8 -*-
"""
Created on Thu May 28 19:37:10 2020
...
CNN Autoencoder for 
    ...
    >MSE error calculation and print
...
num_classes = Xtrain_orig.shape[2]
#==============================================================================
#==========================>   CONV Implementation  <==========================
#==============================================================================
...
#>>>>>   Layer 4 : output layer -----------------------------------------------
        model_c.add(Dense(5,activation = auxi_act_funct))
        model_c.add(Dense(num_classes, activation = output_act))
        
        
        
        model_c.compile (loss=loss_funct , optimizer = adam , \
        ...
        callback = tf.keras.callbacks.EarlyStopping(monitor='val_loss',\
                                                        ...
                                                        patience = 3,\
                                                        mode  = 'min')
        history_c = model_c.fit (Xtrain_compact,Xtrain_orig, \
                     epochs = num_epochs, \
                     batch_size = batch, \
...
#        globals()['dense100_layer_weights_'+str(i)] = weight_get(model_c.layers,1)
        ...
        #ts_result = time.time()
        yh_Xtrain_c = model_c.predict(Xtrain_compact)
        ...
        yh_valid_c = model_c.predict(Xvalid_compact)
        ...
        # ac_test_c_mean_intime[i+1] = (sum(ac_valid_c_mean_intime) + ac_valid_c[i]) / (i+1)
           #______________________________________________________________________ 
        yh_test_c = model_c.predict(Xtest_compact)
...
# yh_Xtrain_c =  yh_Xtrain_c.reshape(yh_Xtrain_c.shape[0],1,yh_Xtrain_c.shape[1])
E_train = yh_Xtrain_c - Xtrain_orig
...
# yh_test_c =  yh_test_c.reshape(yh_test_c.shape[0],1,yh_test_c.shape[1])
E_test = yh_test_c -Xtest_orig
