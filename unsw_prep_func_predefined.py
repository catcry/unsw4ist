 # -*- coding: utf-8 -*-
"""
Created on Mon May 25 20:43:15 2020

@author: catcry

            

    >>  Predefined UNSW-NB15  DATA preparation FUNC     <<
    > Loading
    > Handling missing data
    > processing port number in HEx
...
# faddr_data = r'F:\#DataSets\UNSW-NB15\CSVs\UNSW_NB15_training-set.csv'
# faddr_unique = r'F:\#DataSets\UNSW-NB15\CSVs\UNSW_NB15_testing-set.csv'
...
class Numifi():
    import pandas as pd
    def __init__(self,data,data_unique,name):
        self.name = name
        self.data = data
...
#        self.len = len()
    ...
    def convtonum(self):
        ...
        while i<len(self.name):
            j=0         
            uniq = self.data_unique[self.name[i]].unique()
            # self.data[self.name[i]].unique()
            while j<len(uniq):
...
def unsw_prep (faddr_data,faddr_unique,seperator,\
               vald,vald_percentage,test,test_percentage,\
               binary_class):
    
    x = pd.read_csv(faddr_data,sep=seperator)
    x_unique = pd.read_csv(faddr_unique,sep=seperator)#Reading csv File
   
    #==========================================================================
    #-------------------->>>       cleaning dataset       <<<------------------
    #==========================================================================
    
    x = x.drop(columns = ['id'])  #Drop Source and Dest IPs
    ...
    #___________________>    Converting hex in string to int   <_______________
            ...
            #   In Predefined SETS there are no SOURCE AND DEST PORTS
...
#        globals()['missin_dash_index_'+i] = data.index.values.astype(int)[x[i]=='-']
    ...
    do = Numifi(x,x_unique,['proto','service','attack_cat']) 
    data = do.convtonum()
    #%%    >>  Manually Nomifing  state
    data = data.replace({'state' : 'FIN'}, 0)
    data = data.replace({'state' : 'INT'}, 1)
    data = data.replace({'state' : 'CON'}, 2)
    data = data.replace({'state' : 'ECO'}, 3)
    data = data.replace({'state' : 'REQ'}, 4)
    data = data.replace({'state' : 'RST'}, 5)
    data = data.replace({'state' : 'PAR'}, 6)
    data = data.replace({'state' : 'URN'}, 7)
    data = data.replace({'state' : 'ACC'}, 8)
    data = data.replace({'state' : 'CLO'}, 9)
    data = data.replace({'state' : 'no'}, 10)
